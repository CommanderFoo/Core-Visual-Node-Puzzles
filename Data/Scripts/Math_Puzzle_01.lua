...
local output_first_complete = false

local showing_result_ui = false
local total_puzzle_score = 0

p_evts[#p_evts + 1] = API.Puzzle_Events.on("output_first_complete", function(errors)
	output_first_complete = true
end)

function Tick()
	if(output_first_complete) then
		show_result()
	end
end
