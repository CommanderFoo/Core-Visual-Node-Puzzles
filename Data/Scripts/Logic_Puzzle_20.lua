...
local output_circle_triangle_complete = false
local output_ordered_circle_plus_complete = false
local output_ordered_plus_square_complete = false

local showing_result_ui = false
local total_puzzle_score = 0

p_evts[#p_evts + 1] = API.Puzzle_Events.on("output_circle_triangle_complete", function(errors)
	output_circle_triangle_complete = true
end)

p_evts[#p_evts + 1] = API.Puzzle_Events.on("output_ordered_circle_plus_complete", function(errors)
	output_ordered_circle_plus_complete =  true
end)
...
function Tick()
	if(output_circle_triangle_complete and output_ordered_circle_plus_complete and output_ordered_plus_square_complete) then
		show_result()
	end
end
...
evts[#evts + 1] = Events.Connect("puzzle_edit", function()
	output_circle_triangle_complete = false
	output_ordered_circle_plus_complete = false
